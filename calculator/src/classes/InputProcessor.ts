...
class InputProcessor extends Calculator {
  ...
  // Helper function to count the digit of number
  ...
  insertModifier(state: State, id: string, key: string): Partial<State> {
    ...
    const
      isZero: boolean = /^[0]$/.test(currNot),
      isNumber: boolean = this.isNumber(currNot),
      hasDecimal: boolean = /[.]/.test(currNot),
      hasNegative: boolean = /^[-]/.test(currNot);
